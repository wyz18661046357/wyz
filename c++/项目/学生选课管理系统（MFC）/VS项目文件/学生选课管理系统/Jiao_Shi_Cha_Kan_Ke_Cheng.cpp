// Jiao_Shi_Cha_Kan_Ke_Cheng.cpp: 实现文件
...
IMPLEMENT_DYNAMIC(Jiao_Shi_Cha_Kan_Ke_Cheng, CDialogEx)

Jiao_Shi_Cha_Kan_Ke_Cheng::Jiao_Shi_Cha_Kan_Ke_Cheng(CWnd* pParent /*=nullptr*/)
	: CDialogEx(IDD_Cha_Kan_Ke_Cheng1, pParent)
{

}

Jiao_Shi_Cha_Kan_Ke_Cheng::~Jiao_Shi_Cha_Kan_Ke_Cheng()
{

}

void Jiao_Shi_Cha_Kan_Ke_Cheng::DoDataExchange(CDataExchange* pDX)
{
	CDialogEx::DoDataExchange(pDX);
	DDX_Control(pDX, IDC_LIST1, Fa_Bu_De_Ke_Cheng);
}

BEGIN_MESSAGE_MAP(Jiao_Shi_Cha_Kan_Ke_Cheng, CDialogEx)
//	ON_BN_CLICKED(IDCANCEL, &Jiao_Shi_Cha_Kan_Ke_Cheng::OnBnClickedCancel)
END_MESSAGE_MAP()

// Jiao_Shi_Cha_Kan_Ke_Cheng 消息处理程序
int Jiao_Shi_Cha_Kan_Ke_Cheng::CalcCount(int n, char(*str)[10])//读出一共有多少个课程发布
{
...
BOOL Jiao_Shi_Cha_Kan_Ke_Cheng::OnInitDialog()
{
	...
	cfont1.CreatePointFont(110, _T("宋体"), NULL);
	pWnd1->SetFont(&cfont1);

	CString str[]{ L"  ",L"课程名称",L"任课老师" ,L"上课地点" ,L"上课时间" };
	Fa_Bu_De_Ke_Cheng.InsertColumn(0, str[0], LVCFMT_CENTER, 45);
	for (int i = 1; i < 5; i++)
	{
		Fa_Bu_De_Ke_Cheng.InsertColumn(i, str[i], LVCFMT_LEFT, 180);//, LVCFMT_CENTER
	}
	...
	for (int n = 1; n <= cout; n++)
	{
		...
		Fa_Bu_De_Ke_Cheng.InsertItem(add1, na);
		Fa_Bu_De_Ke_Cheng.SetItemText(add1, 1, str_Ke_Cheng_Ming_Cheng);
		Fa_Bu_De_Ke_Cheng.SetItemText(add1, 2, str_Re_Ke_Jiao_Shi);
		Fa_Bu_De_Ke_Cheng.SetItemText(add1, 3, str_Shang_Ke_Di_Dian);
		Fa_Bu_De_Ke_Cheng.SetItemText(add1, 4, str_Shang_Ke_Shi_Jian);  
		add1++;
		}
	}
    Fa_Bu_De_Ke_Cheng.SetExtendedStyle(Fa_Bu_De_Ke_Cheng.GetExtendedStyle() | LVS_EX_FULLROWSELECT | LVS_EX_GRIDLINES);
