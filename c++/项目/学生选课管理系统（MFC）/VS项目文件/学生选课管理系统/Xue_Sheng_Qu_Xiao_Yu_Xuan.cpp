// Xue_Sheng_Qu_Xiao_Yu_Xuan.cpp: 实现文件
//
...
IMPLEMENT_DYNAMIC(Xue_Sheng_Qu_Xiao_Yu_Xuan, CDialogEx)

Xue_Sheng_Qu_Xiao_Yu_Xuan::Xue_Sheng_Qu_Xiao_Yu_Xuan(CWnd* pParent /*=nullptr*/)
	: CDialogEx(IDD_Xue_Sheng_Qu_Xiao_Yu_Xuan, pParent)
{

}

Xue_Sheng_Qu_Xiao_Yu_Xuan::~Xue_Sheng_Qu_Xiao_Yu_Xuan()
{
}

void Xue_Sheng_Qu_Xiao_Yu_Xuan::DoDataExchange(CDataExchange* pDX)
{
	CDialogEx::DoDataExchange(pDX);
	DDX_Control(pDX, IDC_LIST1, Qu_Xiao_Yu_Xuan_Ke_Cheng_Biao_Ge);
}


BEGIN_MESSAGE_MAP(Xue_Sheng_Qu_Xiao_Yu_Xuan, CDialogEx)
	ON_BN_CLICKED(IDCANCEL2, &Xue_Sheng_Qu_Xiao_Yu_Xuan::OnBnClickedCancel2)
END_MESSAGE_MAP()


// Xue_Sheng_Qu_Xiao_Yu_Xuan 消息处理程序



// Jiao_Shi_Cha_Kan_Ke_Cheng 消息处理程序
int Xue_Sheng_Qu_Xiao_Yu_Xuan::CalcCount(int n, char(*str)[10])//读出一共有多少个课程发布
{
...
BOOL Xue_Sheng_Qu_Xiao_Yu_Xuan::OnInitDialog()
{
	...
	Qu_Xiao_Yu_Xuan_Ke_Cheng_Biao_Ge.InsertColumn(0, str[0], LVCFMT_CENTER, 45);
	for (int i = 1; i < 5; i++)
	{
		Qu_Xiao_Yu_Xuan_Ke_Cheng_Biao_Ge.InsertColumn(i, str[i], LVCFMT_LEFT, 180);//, LVCFMT_CENTER
	}
	...
	for (int i = 1; i <= cout; i++)
	{
		...
		{	
			if (Yu_Xuan_Xue_Sheng_Zhang_Hao == str_Zhang_Hao_Du_Chu)
			{
				...
				Qu_Xiao_Yu_Xuan_Ke_Cheng_Biao_Ge.InsertItem(add, add1);
				Qu_Xiao_Yu_Xuan_Ke_Cheng_Biao_Ge.SetItemText(add, 1, str_Ke_Cheng_Ming_Cheng);
				Qu_Xiao_Yu_Xuan_Ke_Cheng_Biao_Ge.SetItemText(add, 2, str_Re_Ke_Jiao_Shi);
				Qu_Xiao_Yu_Xuan_Ke_Cheng_Biao_Ge.SetItemText(add, 3, str_Shang_Ke_Di_Dian);
				Qu_Xiao_Yu_Xuan_Ke_Cheng_Biao_Ge.SetItemText(add, 4, str_Shang_Ke_Shi_Jian);
				add++;
			}
		}

	}
	Qu_Xiao_Yu_Xuan_Ke_Cheng_Biao_Ge.SetExtendedStyle(Qu_Xiao_Yu_Xuan_Ke_Cheng_Biao_Ge.GetExtendedStyle() | LVS_EX_FULLROWSELECT | LVS_EX_GRIDLINES);
...
void Xue_Sheng_Qu_Xiao_Yu_Xuan::OnBnClickedCancel2()
{
	// TODO: 在此添加控件通知处理程序代码
	CString str10;
	CString str11;
	int nId, mm = 0;
	//首先得到点击的位置
	POSITION pos = Qu_Xiao_Yu_Xuan_Ke_Cheng_Biao_Ge.GetFirstSelectedItemPosition();
	if (pos == NULL)
	{
	...
	//得到行号，通过POSITION转化
	nId = (int)Qu_Xiao_Yu_Xuan_Ke_Cheng_Biao_Ge.GetNextSelectedItem(pos);
	//	str5.Format(L"%d", nId + 1);
	//	MessageBox(L"您选中第" + str5 + L"行");
		//得到列中的内容（0表示第一列，同理1,2,3...表示第二，三，四...列）
	str10 = Qu_Xiao_Yu_Xuan_Ke_Cheng_Biao_Ge.GetItemText(nId, 1);
	CString a;
	a.Format(L"%s", str10);
	CString name = L"D:\\学生选课管理系统\\各个课程的学生信息\\" + a + L".ini";
	int MAX10 = 50;
	CString str_Zhang_Hao_Du_Chu;
	GetPrivateProfileString(L"当前登录的学生账号信息", L"Zhang_Hao", L"当前登录的学生账号信息", str_Zhang_Hao_Du_Chu.GetBuffer(MAX10), MAX10, L"D:\\学生选课管理系统\\当前登录账号密码信息\\当前登录学生账号.ini");
	WritePrivateProfileString(str_Zhang_Hao_Du_Chu, NULL, NULL, name);
	if (MessageBox(L"取消预选课程成功！", L"提示", MB_ICONEXCLAMATION))
		CDialogEx::OnCancel();
}
